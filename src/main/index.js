...
const OPERATIONS = {
  ...
  aims: (/** @type {any} */ a) => api.aims(store, a?.now ?? Date.now()),
  aim: (/** @type {any} */ a) => api.aim(store, a.id, a.now ?? Date.now()),
  setAim: (/** @type {any} */ a) => api.setAim(store, { ...a, now: a.now ?? Date.now() }),
  updateAim: (/** @type {any} */ a) => api.updateAim(store, a.id, a.fields ?? {}),
  endAim: (/** @type {any} */ a) => api.endAim(store, a.id, a),
  logAim: (/** @type {any} */ a) => api.logAim(store, { ...a, now: a.now ?? Date.now() }),
  links: (/** @type {any} */ a) => api.links(store, { ...(a ?? {}), now: a?.now ?? Date.now() }),
  linkTo: (/** @type {any} */ a) => api.linkTo(store, { ...a, now: a.now ?? Date.now() }),
  unlink: (/** @type {any} */ a) => api.unlink(store, a.id),
